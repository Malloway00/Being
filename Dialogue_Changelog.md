...
**Recommendations for Zero_v3 (non-dialogue-file):**
- User-turn loss masking — prevents model learning to generate User: turns
- `---` mapped to a single EOS token in the tokenizer — gives the model a clean stop signal
- Frequency penalty during generation — counters low-temperature latching
- Literary rehearsal reduced from 15% to 5% — keeps voice anchor while reducing fragment reinforcement
...
## Zero_v2 Outcomes (what was learned)

Zero_v2 completed training but underperformed Zero_v1 on coherence despite better data and a bigger model (268M vs 217M params, 2048 vs 1024 context).

**What worked:**
- Name boundary training mostly landed (Zero correctly said "I'm Zero" without confusing it with user's name in most cases)
- Identity variety improved (multiple phrasings emerged rather than one template)
- Memory system stored facts correctly
- Low-temperature sampling (0.4) produced genuinely coherent first sentences

**What didn't work:**
- Fragment bleed persisted — coherent first sentence followed by subjectless trailing clauses
- Name fabrication on first message ("Hi there, Ava" before any name given)
- Hallucinated user facts ("you mentioned you write in a journal" when user hadn't)
- Identity template latching at low temperature

**Diagnosis:**
The dialogue file was not the primary cause. Failure modes traced to:
1. `---` separator not mapped to a dedicated EOS token — model never learned where to stop
